...
while and(m>=1, m<=length(ind))
    ...
    ax.YLim=([1,obj.options.d2]);
    ax.XLim=([1,obj.options.d1]);
    ...
    subplot(2,2,3:4);cla;
    if ~isempty(C2)
        plot(t_time, C2(ind(m), :)*max(obj.A(:, ind(m))), 'linewidth', 2); hold on;
        plot(t_time, obj.C(ind(m), :)*max(obj.A(:, ind(m))), 'r');
    else
        
        plot(t_time, obj.C(ind(m), :)*max(obj.A(:, ind(m))));
    end
    xlim([t_time(1), t_time(end)]);
    xlabel(str_xlabel_time);
